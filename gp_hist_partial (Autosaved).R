...
(y <- my_hist(x_dat,bounds=alpha))#so x_mle doesn't freak out
...
plot_dens <- function(results,r,T_out, save_pics = FALSE){
  ...
  plot(T_out,mean_est,type = 'l', main = 'GP Density Estimate',
       ...
       ylim = c(0,max(apply(f_est,2,quantile,0.975))))#,ylim = c(0,2))
  ...
  legend('topright',c('Post Mean','Post 95% Cred','Truth'),
